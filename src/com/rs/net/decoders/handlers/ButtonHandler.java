...
public class ButtonHandler {
	...
	public static void handleButtons(final Player player, InputStream stream, int packetId) {
		...
		if (!player.getControlerManager().processButtonClick(interfaceId, componentId, slotId, packetId)) { return; }
		if (interfaceId == 387) {
			...
			} else if (componentId == 8 && packetId == 61) { ButtonHandler.sendRemove(player, Equipment.SLOT_HAT); }
			...
			} else if (componentId == 20 && packetId == 61) { ButtonHandler.sendRemove(player, Equipment.SLOT_CHEST); }

			// Shield Gear
			else if (componentId == 23 && packetId == 25) {
				player.getEquipment().sendExamine(Equipment.SLOT_SHIELD);
			} else if (componentId == 23 && packetId == 61) { ButtonHandler.sendRemove(player, Equipment.SLOT_SHIELD); }

			// Leg Gear
			else if (componentId == 26 && packetId == 25) {
				player.getEquipment().sendExamine(Equipment.SLOT_LEGS);
			} else if (componentId == 26 && packetId == 61) { ButtonHandler.sendRemove(player, Equipment.SLOT_LEGS); }

			// Gloves Gear
			else if (componentId == 29 && packetId == 25) {
				player.getEquipment().sendExamine(Equipment.SLOT_HANDS);
			} else if (componentId == 29 && packetId == 61) { ButtonHandler.sendRemove(player, Equipment.SLOT_HANDS); }

			// Feet Gear
			else if (componentId == 32 && packetId == 25) {
				player.getEquipment().sendExamine(Equipment.SLOT_FEET);
			} else if (componentId == 32 && packetId == 61) { ButtonHandler.sendRemove(player, Equipment.SLOT_FEET); }

			// Rings
			else if (componentId == 35 && packetId == 25) {
				player.getEquipment().sendExamine(Equipment.SLOT_RING);
			} else if (componentId == 35 && packetId == 61) { ButtonHandler.sendRemove(player, Equipment.SLOT_RING); }

			// Arrows
			else if (componentId == 38 && packetId == 25) {
				player.getEquipment().sendExamine(Equipment.SLOT_ARROWS);
			} else if (componentId == 38 && packetId == 61) { ButtonHandler.sendRemove(player, Equipment.SLOT_ARROWS); }
			...
			} else if (componentId == 11 && packetId == 25) { player.getEquipment().sendExamine(Equipment.SLOT_CAPE); }
		}
		if (interfaceId == 548 || interfaceId == 746 || interfaceId == 387) {
			...
			if ((interfaceId == 548 && componentId == 180) || (interfaceId == 746 && componentId == 182)) {
				...
				// xp counter reset
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON7_PACKET) { player.getSkills().resetXpCounter(); }
			}
			...
			} else if (componentId == 21) { Familiar.confirmLeftOption(player); } else if (componentId == 25) {
				Familiar.setLeftclickOption(player, 7);
			}
		} else if (interfaceId == 662) {
			if (player.getFamiliar() == null) { return; }
			if (componentId == 49) { player.getFamiliar().call(); } else if (componentId == 51) {
				player.getDialogueManager().startDialogue("DismissD");
			} else if (componentId == 67) { player.getFamiliar().takeBob(); } else if (componentId == 69) {
			...
			} else if (player.getFamiliar() == null) { return; }
			...
			} else { Magic.processNormalSpell(player, componentId, packetId); }
			...
			if (packetId == 61) { player.getTrade().addItem(player, slotId, 1); } else if (packetId == 64) {
			...
			// player.getTrade().addLendItem(slotId);
				...
				player.getPackets().sendGameMessage((new StringBuilder("A ")).append((new Item(slotId2, 1)).getDefinitions().getName().toLowerCase()).append("'s value is ").append(ItemDefinitions.getItemDefinitions(slotId2).getValue(slotId2)).append(" gold.").toString());
			}
		} else if (interfaceId == 335) {
			if (componentId == 34) {
				if (packetId == 25) { player.getBank().sendExamine(slotId2); } else if (packetId == 61) {
					player.getPackets().sendGameMessage((new StringBuilder("A ")).append((new Item(slotId2, 1)).getDefinitions().getName().toLowerCase()).append("'s value is ").append(ItemDefinitions.getItemDefinitions(slotId2).getValue(slotId2)).append(" gold.").toString());
				}
			}
			if (componentId == 31) {
				if (packetId == 61) { player.getTrade().removeItem(player, slotId, 1); } else if (packetId == 64) {
			...
			} else if (componentId == 18) { player.getCombatDefinitions().switchDefensiveCasting(); } else {
			...
			} else if (componentId == 20) { player.getCombatDefinitions().switchDefensiveCasting(); } else {
			...
			} else if (componentId == 4) { player.switchAllowChatEffects(); } else if (componentId == 5) {
				player.getInterfaceManager().sendSettings(982);
			} else if (componentId == 6) { player.switchMouseButtons(); } else if (componentId == 16) {
				if (player.getInterfaceManager().containsScreenInter()) {
			...
			if (componentId == 5) { player.getInterfaceManager().sendSettings(); } else if (componentId == 42) {
			...
			if (skillMenu != -1) { player.getTemporaryAttributtes().put("skillMenu", skillMenu); }
			...
			if (componentId == 25) {
				...
				} else if (packetId == WorldPacketsDecoder.ACTION_BUTTON8_PACKET) { shop.sendExamine(player, slotId); }
			}
			...
			if (componentId == 0) {
				...
				if (item == null) { return; }
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) {
					if (sendWear(player, slotId, item.getId())) { ButtonHandler.refreshEquipBonuses(player); }
				} else if (packetId == WorldPacketsDecoder.ACTION_BUTTON4_PACKET) {
					player.getInventory().sendExamine(slotId);
				}
			}
			...
			if (componentId == 0) {
				if (slotId > 27 || player.getInterfaceManager().containsInventoryInter()) { return; }
				Item item = player.getInventory().getItem(slotId);
			...
			if (componentId == 1) {
				...
				{ player.getPrayer().switchQuickPrayers(); } else if (packetId == WorldPacketsDecoder.ACTION_BUTTON2_PACKET) // switch
				{ player.getPrayer().switchSettingQuickPrayer(); }
			}
			...
			} else if (componentId == 18) { player.getBank().depositAllInventory(false); } else if (componentId == 20) {
				player.getBank().depositAllEquipment(false);
			}
		} else if (interfaceId == 762) {
			if (componentId == 117) { return; }
			if (componentId == 15) { player.getBank().switchInsertItems(); } else if (componentId == 19) {
			...
			ClanChatManager.handleButtons(player, componentId, packetId, stream);
			
		} else if (interfaceId == 767) {
			if (componentId == 10) { player.getBank().openBank(); }
		} else if (interfaceId == 884) {
			if (componentId == 4) {
				if (player.hasInstantSpecial(player.getEquipment().getWeaponId())) { return; }
			...
			} else if (componentId == 15) { player.getCombatDefinitions().switchAutoRelatie(); }
	...
	public static void sendRemove(Player player, int slotId) {
		...
		if (slotId == 3) { player.getCombatDefinitions().desecreaseSpecialAttack(0); }
	}
	...
	public static boolean sendWear(Player player, int slotId, int itemId) {
		...
		for (String strings : Settings.DONATOR_ITEMS) {
			if (itemName.contains(strings) && !player.isDonator()) {
				player.getPackets().sendGameMessage("You need to be a donator to spawn " + itemName + ".");
				return true;
			}
		}
		...
		if (!player.getControlerManager().canEquip(targetSlot, itemId)) { return false; }
		...
		if (targetSlot == 3) { player.getCombatDefinitions().desecreaseSpecialAttack(0); }
	...
	public static void sendWear(Player player, int[] slotIds) {
		...
		for (int slotId : slotIds) {
			...
			if (sendWear2(player, slotId, item.getId())) { worn = true; }
		}
	...
	public static boolean sendWear2(Player player, int slotId, int itemId) {
		...
		if (!hasRequiriments) { return false; }
		if (!player.getControlerManager().canEquip(targetSlot, itemId)) { return false; }
